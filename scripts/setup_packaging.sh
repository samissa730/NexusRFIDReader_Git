...
if command -v python3 &> /dev/null; then
    ...
    if [ "$PYTHON_MAJOR" -eq 3 ] && [ "$PYTHON_MINOR" -ge 9 ]; then
        echo -e "   ${GREEN}✓${NC} Python version is compatible (3.9+)"
    else
        echo -e "   ${YELLOW}⚠️  Python version may need upgrade (3.9+ recommended)${NC}"
    fi
else
...
if command -v pyinstaller &> /dev/null; then
    ...
    echo -e "   ${GREEN}✓${NC} PyInstaller found: $PYINSTALLER_VERSION"
else
    echo -e "   ${YELLOW}⚠️  PyInstaller not found - will be installed during packaging${NC}"
fi

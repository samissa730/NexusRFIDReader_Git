...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="GPS Connection and Reading Test with Detailed Logging",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Auto-detect GPS port
  python gps_test.py --auto-detect
  
  # Connect to specific port
  python gps_test.py --port COM3 --baud 115200
  
  # Linux port with custom baud rate
  python gps_test.py --port /dev/ttyUSB0 --baud 9600
  
  # Show all NMEA sentences and raw data
  python gps_test.py --port COM3 --show-all --show-raw
  
  # Save logs to file
  python gps_test.py --port COM3 --log-file gps_test.log
        """
    )
    ...
    parser.add_argument(
        '--auto-detect', '-a',
        action='store_true',
        help='Auto-detect GPS port and baud rate'
    )
    parser.add_argument(
        '--list-ports', '-l',
        action='store_true',
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("GPS Connection and Reading Test")
    logger.info("=" * 60)
